%% Function to calculate the N2gas concentration. Taken from Marzadri 2011
...
% C_3_s: Initial concentration of N2gas in the stream [mg/L]
...
function [C_3_matrix]=compute_N2gas_Concentration(C_0_s,C_1_s,C_2_s,C_3_s,C_0_lim,tau_lim,KR,KN,KC,KD,t,tau,Pulse,DL,u)
...
if DL==0
    ...
    C_3_matrix=C_3_s.*ones(size(t_matrix));
    % % Anaerobic conditions. KC=0, KN=0, KD>0, tau>tau_lim
    C_2_lim=C_2_s.*exp(-KC.*tau_lim)+C_1_s.*(KN/(KC-KN)).*(exp(-KN.*tau_lim)-exp(-KC.*tau_lim)); % Correction!
    C_3_matrix_an=C_3_s+C_2_lim.*(1-exp(-KD.*(tau_matrix-tau_lim)));
    C_3_matrix(tau_matrix>tau_lim)=C_3_matrix_an(tau_matrix>tau_lim);
    % For t<tau
    C_3_matrix(t_matrix<tau_matrix)=0;

else

    % Approach using tau_lim as a constant
    %     % For tau>tau_lim
    % %     C3=exp(u.^2.*tau_matrix./DL).*erfc((u.*tau_matrix+u.*t_matrix)./(2.*sqrt(DL.*t_matrix))) + erfc((u.*tau_matrix-u.*t_matrix)./(2.*sqrt(DL.*t_matrix))) -...
    % %         exp((u^2*tau_matrix./(2*DL)).*(1+sqrt(1+4*KD*DL./(u^2)))).*erfc((u.*tau_matrix+u.*t_matrix.*sqrt(1+4*KD*DL/(u^2)))./(2*sqrt(DL.*t_matrix)))-...
    % %         exp((u^2*tau_matrix./(2*DL)).*(1-sqrt(1+4*KD*DL./(u^2)))).*erfc((u.*tau_matrix-u.*t_matrix.*sqrt(1+4*KD*DL/(u^2)))./(2*sqrt(DL.*t_matrix)));
    %
    %         C3=exp(u.^2.*(tau_matrix-tau_lim)./DL).*erfc((u.*(tau_matrix-tau_lim)+u.*t_matrix)./(2.*sqrt(DL.*t_matrix))) + erfc((u.*(tau_matrix-tau_lim)-u.*t_matrix)./(2.*sqrt(DL.*t_matrix))) -...
    %         exp((u^2*(tau_matrix-tau_lim)./(2*DL)).*(1+sqrt(1+4*KD*DL./(u^2)))).*erfc((u.*(tau_matrix-tau_lim)+u.*t_matrix.*sqrt(1+4*KD*DL/(u^2)))./(2*sqrt(DL.*t_matrix)))-...
    %         exp((u^2*(tau_matrix-tau_lim)./(2*DL)).*(1-sqrt(1+4*KD*DL./(u^2)))).*erfc((u.*(tau_matrix-tau_lim)-u.*t_matrix.*sqrt(1+4*KD*DL/(u^2)))./(2*sqrt(DL.*t_matrix)));
    %
    %
    ...
    %
    %     C_3_matrix=0.5.*C_2_lim .*C3;
    %
    %     % For tau<tau_lim
    %     C_3_matrix(tau_matrix<tau_lim)=0;
    ...
    C3=exp(u.^2.*(tau_matrix-tau_lim_matrix)./DL).*erfc((u.*(tau_matrix-tau_lim_matrix)+u.*t_matrix)./(2.*sqrt(DL.*t_matrix))) + erfc((u.*(tau_matrix-tau_lim_matrix)-u.*t_matrix)./(2.*sqrt(DL.*t_matrix))) -...
    ...
    C_1_ox_lim=exp((u^2*tau_lim_matrix./(2*DL)).*(1+sqrt(1+4*KN*DL./(u^2)))).*erfc((u.*tau_lim_matrix+u.*tau_lim_matrix.*sqrt(1+4*KN*DL/(u^2)))./(2*sqrt(DL.*tau_lim_matrix)))+...
        exp((u^2*tau_lim_matrix./(2*DL)).*(1-sqrt(1+4*KN*DL./(u^2)))).*erfc((u.*tau_lim_matrix-u.*tau_lim_matrix.*sqrt(1+4*KN*DL/(u^2)))./(2*sqrt(DL.*tau_lim_matrix)));


    C_1_matrix_lim=0.5.*C_1_s.*C_1_ox_lim;

    C_2_ox_lim=exp((u^2*tau_lim_matrix./(2*DL)).*(1+sqrt(1+4*KC*DL./(u^2)))).*erfc((u.*tau_lim_matrix+u.*tau_lim_matrix.*sqrt(1+4*KC*DL/(u^2)))./(2*sqrt(DL.*tau_lim_matrix)))+...
        exp((u^2*tau_lim_matrix./(2*DL)).*(1-sqrt(1+4*KC*DL./(u^2)))).*erfc((u.*tau_lim_matrix-u.*tau_lim_matrix.*sqrt(1+4*KC*DL/(u^2)))./(2*sqrt(DL.*tau_lim_matrix)));
    ...
    % For tau<tau_lim
    C_3_matrix(tau_matrix<tau_lim_matrix)=0;
